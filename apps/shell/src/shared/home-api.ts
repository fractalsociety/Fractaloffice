...
/** UI language; kept self-contained here (mirrors Lang in @fractal-office/i18n) */
...
export interface HomeApi {
  ...
  /** Codex account status (codex login state; to be upgraded to a signup/account system later) */
  accountStatus(): Promise<AccountStatus>
  /** start Codex login (opens the browser; accountStatus flips to logged-in on completion); returns whether the launch succeeded */
  accountLogin(): Promise<boolean>
  /** log out (clears the saved API key; the login state is shared globally with the codex CLI) */
  ...
  /** shared AI provider/model settings used by every editor */
