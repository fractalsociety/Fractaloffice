...
import { createI18n, isLang, normalizeLang, setUiLang, type Lang } from '@fractal-office/i18n'
import { installNavigationGuard } from '@fractal-office/electron-utils'
...
/**
 * Fractal Office unified shell: ONE Electron app, ONE BrowserWindow, hosting the
...
// run silently quits and forwards its argv to the running installed Fractal Office.
// FRACTAL_OFFICE_USER_DATA: test drivers point this at a scratch dir so an
// automated instance can run alongside the dev instance (separate lock).
if (!app.isPackaged)
  app.setPath(
    'userData',
    process.env.FRACTAL_OFFICE_USER_DATA ?? join(app.getPath('appData'), 'Fractal Office Dev'),
  )

// The product rename from "AI Office" to Fractal Office changed the userData path; migrate old user data once
if (app.isPackaged) {
...
// same file when they pick up i18n later. FRACTAL_OFFICE_LANG overrides for tests.
...
function currentLang(): Lang {
  ...
  if (process.env.FRACTAL_OFFICE_LANG) {
    uiLang = normalizeLang(process.env.FRACTAL_OFFICE_LANG)
    setUiLang(uiLang)
    return uiLang
  }
...
const tMain = createI18n({
  zh: {
    ...
    menuSectionNew: '新建',
    menuNewDoc: 'AI Docs',
    menuNewSheet: 'Fractal Office Sheets',
    untitledSheet: '未命名表格',
    untitledDoc: '未命名文档',
    untitledDeck: '未命名演示文稿',
    menuNewSlide: 'Fractal Office Slides',
    menuOpen: '打开…',
    menuSave: '保存',
    menuSaveAs: '另存为…',
    menuClose: '关闭',
...
function registerHomeIpc(): void {
  // Local Codex CLI status. No hosted office account is required.
  ipcMain.handle(HOME_CHANNELS.accountStatus, async () => {
    try {
      resolveCodexCliPath()
      return { loggedIn: true, email: 'Local Codex CLI' }
    } catch {
      return { loggedIn: false }
    }
  })

  ipcMain.handle(HOME_CHANNELS.accountLogin, () => {
