...
import { AI_PROVIDERS, type AiProviderId, type AiSettings } from '@fractal-office/ai-provider'
import logoLockup from './assets/fractal-office-logo.svg'
...
const LOCAL_ENDPOINT_PRESETS = [
  ...
  { label: 'Hermes Proxy', url: 'http://127.0.0.1:8645/v1' },
] as const
...
function AiModelDialog({ onClose }: { onClose: () => void }) {
  ...
  const meta = draft ? AI_PROVIDERS.find((provider) => provider.id === draft.provider) : undefined
  const config = draft && meta ? draft.providers[meta.id] : undefined
  ...
  const updateConfig = (patch: Partial<{ model: string; baseUrl: string; apiKey: string }>) => {
    setDraft((current) => {
  ...
  const save = async () => {
    if (!draft || !meta || !config) return
    if (!config.model.trim()) {
      setError('Enter the exact model name exposed by your local server.')
      return
    }
    if (meta.needsBaseUrl && !config.baseUrl?.trim()) {
      setError('Enter the OpenAI-compatible endpoint URL.')
      return
    }
    if (meta.requiresApiKey !== false && meta.id !== 'codex' && !config.apiKey.trim()) {
      setError('Enter the API key for this provider.')
      return
    }
    setSaving(true)
    try {
  ...
  return (
    <div className="ai-model-overlay" onMouseDown={onClose}>
      <div
        className="ai-model-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="ai-model-head">
          <div>
            <span className="ai-model-kicker">Fractal Intelligence</span>
            <h2>AI model</h2>
            <p>Choose Codex or connect an OpenAI-compatible Hermes/local model server.</p>
          </div>
          <button className="ai-model-close" onClick={onClose} aria-label="Close">
            ×
          </button>
        </div>

        {!draft ? (
          <div className="ai-model-loading">{error || 'Loading model settings…'}</div>
        ) : (
          <>
            <div className="ai-provider-grid">
              {AI_PROVIDERS.map((provider) => (
                <button
                  key={provider.id}
                  className={`ai-provider-card${draft.provider === provider.id ? ' active' : ''}`}
              ...
              <div className="ai-model-form">
                {meta.id === 'hermes' && (
                  <div className="ai-local-note">
                    <strong>Runs against your endpoint.</strong> Fractal Office sends document-tool
                    requests directly to the local server you choose. Start the model server first.
                  </div>
                )}
                ...
                {meta.id === 'hermes' && (
                  <div className="ai-endpoint-presets" aria-label="Local endpoint presets">
                    {LOCAL_ENDPOINT_PRESETS.map((preset) => (
                      <button
                        key={preset.label}
                        onClick={() => updateConfig({ baseUrl: preset.url })}
                      >
                        {preset.label}
                      </button>
                    ))}
                ...
                <label>
                  ...
                  <input
                    ...
                    placeholder={meta.id === 'hermes' ? 'Example: qwen3.5:9b' : meta.defaultModel}
                    spellCheck={false}
                  />
                ...
                {meta.id !== 'codex' && (
                  <label>
                    <span>API key {meta.requiresApiKey === false && <em>optional</em>}</span>
                    <input
                      type="password"
                      value={config.apiKey}
                ...
                {meta.id === 'codex' && (
                  <div className="ai-local-note codex-note">
                    Uses your authenticated local Codex CLI. No API key is stored in Fractal Office.
                  </div>
                )}
              </div>
            )}

            {error && <div className="ai-model-error">{error}</div>}

            <div className="ai-model-actions">
              <button className="btn btn-secondary" onClick={onClose}>
                Cancel
              </button>
              <button className="btn ai-model-save" disabled={saving} onClick={() => void save()}>
...
// Currently the Codex (codex) login entry; to be upgraded to a signup/account system later.
...
function AccountEntry() {
  ...
  return (
    <div className="account-entry">
        ...
        <div className="account-menu" role="menu">
          ...
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
              <path
                d="M8 1.7 9.2 5l3.4 1.2L9.2 7.4 8 10.8 6.8 7.4 3.4 6.2 6.8 5 8 1.7Z"
                stroke="currentColor"
                strokeWidth="1.15"
                strokeLinejoin="round"
              />
              <path
                d="m12.6 10 .55 1.45 1.45.55-1.45.55L12.6 14l-.55-1.45L10.6 12l1.45-.55L12.6 10Z"
                fill="currentColor"
              />
            </svg>
            <span>AI model</span>
      ...
      {aiDialogOpen && <AiModelDialog onClose={() => setAiDialogOpen(false)} />}
    </div>
  )
}
    ...
    <div className="home">
      <aside className="sidebar">
        <div className="sidebar-logo">
          <img className="logo-lockup" src={logoLockup} alt="Fractal Office" />
        </div>
