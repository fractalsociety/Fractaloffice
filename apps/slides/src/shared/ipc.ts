 ...
 */
import type { RenderSlide } from '@fractal-office/pptx-render'
import type { SlideComment, SectionInfo } from '@fractal-office/pptx-engine'
import type {
  AiSettings,
  AiStreamChunk,
  AiStreamRequest,
...
export type { SlideComment, SectionInfo } from '@fractal-office/pptx-engine'

// Canonical definitions of AI-related types live in @fractal-office/ai-provider / @fractal-office/agent-core (shared with docs)
export type {
  AiProviderConfig,
  AiProviderId,
  AiProviderMeta,
...
export { AI_PROVIDERS } from '@fractal-office/ai-provider'
export type { AgentToolCall, AgentToolDef } from '@fractal-office/agent-core'
...
export interface SlidesApi {
  ...
  htmlToPptx: (
    ...
    | { error: string }
  >
  /** Whether cloud single-page generation (codex slide_generate) is available (FRACTAL_OFFICE_CLOUD_SLIDE=1 + codex login) */
  ...
  /** Codex account status (codex login state); with withEmail also fetches the email (needs a network request, slower) */
  ...
  /** codex (Codex) AI image generation/editing, returns the image URL (error prompts login when logged out) */
  generateImage: (op: {
    prompt: string
    model?: string
  ...
  /** codex (Codex) media analysis: image/audio/video content understanding, returns analysis text */
  analyzeMedia: (op: {
    mediaUrls: string[]
    requirements: string
  }) => Promise<{ text?: string; error?: string }>
  /** codex availability: installed and logged in (for UI/tools to prompt login) */
  codexStatus: () => Promise<{ available: boolean; email?: string }>
