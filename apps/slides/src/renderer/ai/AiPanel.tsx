...
import {
  ...
  type AgentImage,
  type ToolDisplay,
} from '@fractal-office/agent-core'
...
export function AiPanel({
  ...
  if (!loopRef.current) {
    ...
    const access: DeckAccess = {
      ...
      // Cloud single-page generation (codex slide_generate): the cloud service owns HTML writing +
  ...
  const runWith = (instruction: string, displayText?: string, opts?: { slideShot?: boolean }) => {
    ...
    void Promise.all([window.slidesApi.getAiSettings(), collectImageAttachments()])
      .then(async ([latestSettings, images]) => {
            ...
            >
              ...
              {entry.loginRequired && (
                <button
                  ...
                  onClick={() => void window.slidesApi.aiCodexLogin()}
                >
                  {t('aiCodexLoginBtn')}
                </button>
              )}
