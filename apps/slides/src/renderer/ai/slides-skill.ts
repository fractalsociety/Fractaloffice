import type { AgentSkill, ToolDisplay } from '@fractal-office/agent-core'
import type {
  GroupRenderNode,
  PictureRenderNode,
  RenderNode,
  RenderSlide,
  ShapeRenderNode,
} from '@fractal-office/pptx-render'
...
export interface DeckAccess {
  ...
  /**
   * Cloud single-page generation (codex slide_generate), used by generate_deck's self-driven
...
const AGENT_SYSTEM_PROMPT = `You are the AI assistant inside Fractal Office Slides (a slide editor), helping users improve and generate presentations.
...
const TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'generate_image',
    description:
      'AI image generation/editing (Codex). Text-to-image, or pass referenceImageUrls for image editing; returns an image URL, then insert with insert_web_image. Use for custom illustrations/icons/backgrounds, style-consistent imagery, and edits like background removal/upscaling/outpainting; for real photos/screenshots still use image_search.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
  ...
  },
  {
    name: 'analyze_media',
    description:
      'Analyze media content (Codex): understand images/audio/video. Pass media URLs (or local file paths) and analysis requirements; returns analysis text. Video supports extracting key points, structure, and time ranges — good for turning user material into usable deck content.',
    inputSchema: {
      type: 'object',
      properties: {
        mediaUrls: {
...
async function executeTool(
  ...
  switch (call.name) {
    ...
    case 'generate_deck': {
      ...
      //   2) Generate: batched concurrent cloud page generation (codex slide_generate, one retry per page), **each batch lands immediately → frontend shows pages one by one**.
      if (!access.generatePageCloud || !(await access.isCloudPageGenEnabled?.().catch(() => false)))
        return fail(
          t('aiFailGenDeck'),
          'Cloud slide generation is unavailable — sign in to Codex (codex) first',
        )
      ...
      // The cloud service (codex slide_generate) writes each page's HTML and converts it to a
