/**
 * Fractal Office Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
import { installNavigationGuard, safeExternalUrl } from '@fractal-office/electron-utils'
import { getUiLang, normalizeLang, setUiLang } from '@fractal-office/i18n'
...
import {
  ...
  type Slide,
  type TextElement,
} from '@fractal-office/pptx-engine'
import { buildRenderSlide, EMU_PER_PX_96, type RenderSlide } from '@fractal-office/pptx-render'
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/Fractal Office):
...
/** Directory where AI-generated drafts are saved: <Documents>/Fractal Office/ */
function getDraftsDir(): string {
  return join(app.getPath('documents'), 'Fractal Office')
}
...
/**
 * Auto-save the draft to <Documents>/Fractal Office/<name>.pptx after AI generation completes.
...
export function registerSlidesIpc(): void {
  ...
  // ── Hosted single-page generation is disabled in this local-only build.
  ...
  // converting. Enabled when codex is logged in; FRACTAL_OFFICE_CLOUD_SLIDE=0 is the kill switch.
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return { ok: false, error: 'Hosted slide generation is disabled in this local-only build.' }
    },
  )
  ...
  ipcMain.handle('slides:copy-slide', (e, slideIndex: number, pngBase64?: string) => {
    ...
    clipboard.writeBuffer('io.fractal-office.slides.slide', Buffer.from('1'))
    return true
  })
  ...
  ipcMain.handle('slides:clipboard-external', () => {
    ...
    if (slideClipboard && marker('io.fractal-office.slides.slide')) return { kind: 'slide' }
    if (marker('io.fractal-office.slides.elements')) return { kind: 'internal' }
  ...
  ipcMain.handle('slides:copy-elements', (e, op: CopyElementsOp) => {
    ...
    if (items.length) {
      ...
      clipboard.writeBuffer('io.fractal-office.slides.elements', Buffer.from('1'))
    }
    return items.length
  })
...
export function startSlidesStandalone(): void {
  ...
  // FRACTAL_OFFICE_USER_DATA: test drivers point this at a scratch dir so automated
  ...
  if (!app.isPackaged && process.env.FRACTAL_OFFICE_USER_DATA) {
    app.setPath('userData', process.env.FRACTAL_OFFICE_USER_DATA)
  }
