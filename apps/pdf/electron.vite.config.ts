...
export default defineConfig({
  // @fractal-office/i18n ships as TS source; pdf-lib's package only includes out/** — both must be bundled
  main: {
    plugins: [
      externalizeDepsPlugin({
        exclude: ['@fractal-office/i18n', 'pdf-lib', '@fractal-office/electron-utils'],
      }),
    ],
  },
  preload: {
    plugins: [externalizeDepsPlugin({ exclude: ['@fractal-office/i18n'] })],
  },
  renderer: {
    plugins: [
      react(),
