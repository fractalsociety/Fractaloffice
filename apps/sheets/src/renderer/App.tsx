...
export function App(): React.JSX.Element {
    ...
    agentLoopRef.current = new AgentLoop({
      ...
      events: {
        ...
        onError: (error) => {
          ...
          if (aiSettingsRef.current?.provider === 'codex') {
  ...
  function isAgentConfigured(): boolean {
    ...
    return settings.provider === 'codex' || settings.provider === 'hermes' || !!config.apiKey
  }
  ...
  function runAgent(instruction: string): void {
    ...
    void Promise.all([window.desktopApi.getAiSettings(), collectImageAttachments()])
      .then(([latestSettings, images]) => {
        aiSettingsRef.current = latestSettings
        setAiSettingsState(latestSettings)
