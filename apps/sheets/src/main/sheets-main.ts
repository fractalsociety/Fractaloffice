...
import { createI18n, getUiLang, type Lang, normalizeLang, setUiLang } from '@fractal-office/i18n'
...
/** office/pdf formats extract text via @fractal-office/file-parse; images skip text
...
/** Attachment text extraction via @fractal-office/file-parse (docx/pdf/pptx/xlsx/plain text) */
...
export function registerSheetsAiIpc(): void {
  ...
  ipcMain.handle(
    IPC_CHANNELS.aiCodexStatus,
    async (_event, withEmail?: unknown): Promise<CodexAccountStatus> => {
      try {
        resolveCodexCliPath()
  ...
  ipcMain.handle(IPC_CHANNELS.aiCodexLogin, () => {
...
async function prepareWorkbookForOpen(
  ...
  const directory = join(app.getPath('temp'), 'fractal-office-imports', randomUUID())
