...
describe('findDocxPath', () => {
  it('finds Finder and Explorer document arguments case-insensitively', () => {
    expect(
      findDocxPath(['/Applications/Fractal Office Docs.app', '/tmp/Quarterly Plan.docx']),
    ).toBe('/tmp/Quarterly Plan.docx')
    expect(findDocxPath(['Fractal Office Docs.exe', 'C:\\Users\\Me\\REPORT.DOCX'])).toBe(
  ...
  it('ignores Electron switches and unrelated files', () => {
    expect(
      findDocxPath(['Fractal Office Docs', '--inspect=document.docx', '/tmp/notes.txt']),
    ).toBeNull()
  })
})
