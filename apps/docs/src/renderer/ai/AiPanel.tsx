...
import type { Block } from '@fractal-office/docx-engine'
import { AgentLoop, composeSkills, type AgentImage } from '@fractal-office/agent-core'
...
import { Markdown } from '@fractal-office/ui'
import { AiComposer, AiTypingIndicator } from '@fractal-office/ui'
...
interface ChatEntry {
  ...
  undelivered?: boolean
  /** the run failed because Codex is signed out — render an inline sign-in button (#87) */
...
export function AiPanel({
    ...
    loopRef.current = new AgentLoop<PmNode>({
      ...
      events: {
        ...
        onError: (error) => {
          ...
          if (settingsRef.current.provider === 'codex') {
            void window.desktop
              .aiCodexStatus()
              .then((status) => {
  ...
  const runWith = (instruction: string, displayInstruction = instruction) => {
    ...
    void Promise.all([window.desktop.getAiSettings(), collectImageAttachments()]).then(
      ([latestSettings, images]) => {
        ...
        {chat.map((entry, i) => {
          ...
          return (
            ...
            >
              ...
              {entry.loginRequired && (
                <button className="ai-login-btn" onClick={() => void window.desktop.aiCodexLogin()}>
                  {t('aiCodexLoginBtn')}
                </button>
              )}
              {showToolbar && (
