# Contributing to Fractal Office
...
| Variable                                                 | Effect                                                                 |
| -------------------------------------------------------- | ---------------------------------------------------------------------- |
| `FRACTAL_OFFICE_USER_DATA`                               | Override the Electron userData directory (test isolation)              |
| `FRACTAL_OFFICE_LANG`                                    | Force the UI language instead of following the OS locale               |
| `CODEX_CLI_PATH`                                         | Absolute path to the authenticated local Codex CLI                     |
| `SERPER_API_KEY`                                         | Optional Serper key for web/image search; DuckDuckGo is the fallback   |
...
If the Codex CLI is missing, AI requests surface an inline setup prompt. Web
search falls back to a keyless backend when Serper is not configured.
