...
Please report suspected vulnerabilities privately to the Fractal Office
maintainers. Do not open public issues for security reports. We aim to
...
- Every `shell.openExternal` call goes through a single shared gate
  (`@fractal-office/electron-utils` → `safeExternalUrl`) that parses the URL and
...
- The locally installed Codex CLI and optional direct model/search providers
  are separate products; issues in those services should be reported to their
...
- Vulnerabilities that require an already-compromised machine or a modified
  ...
  for local development (`CODEX_CLI_PATH`, `XLSX_SIDECAR_PATH`): setting them
