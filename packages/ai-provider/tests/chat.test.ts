...
describe('chatForProvider', () => {
  ...
  it('anthropic: replaces an HTML error body with a readable note', async () => {
    const html =
      '<!doctype html>\n<html>\n<head><title>Login</title></head><body>app shell</body></html>'
  ...
  it('hermes: supports a keyless local OpenAI-compatible endpoint', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ choices: [{ message: { content: 'local result' } }] }))
    vi.stubGlobal('fetch', fetchMock)
    const result = await chatForProvider(
      'hermes',
      { apiKey: '', model: 'qwen3.5:9b', baseUrl: 'http://127.0.0.1:11434/v1/' },
      'sys',
      'hi',
    )
    expect(result).toEqual({ ok: true, content: 'local result' })
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it('hermes: rejects without a local base URL', async () => {
    const fetchMock = vi.fn()
    vi.stubGlobal('fetch', fetchMock)
    const result = await chatForProvider('hermes', { apiKey: '', model: 'local' }, 'sys', 'hi')
    expect(result).toEqual({ ok: false, error: 'Hermes requires a local Base URL' })
