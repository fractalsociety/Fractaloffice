...
import type { AgentToolCall } from '@fractal-office/agent-core'
...
describe('streamForProvider: anthropic', () => {
  ...
  it('replaces an HTML error body (e.g. a gateway block page) with a readable note', async () => {
    const html =
      '<!doctype html>\n<html>\n<head><title>Codex</title></head><body>app shell</body></html>'
...
describe('streamForProvider: openai-compatible', () => {
  ...
  it('streams from a keyless Hermes/local endpoint without an Authorization header', async () => {
    const fetchMock = vi.fn().mockResolvedValue(okResponse(sseStream(['data: [DONE]'])))
    vi.stubGlobal('fetch', fetchMock)
    const { cb } = collector()
    await streamForProvider(
      'hermes',
      { apiKey: '', model: 'local-model', baseUrl: 'http://127.0.0.1:1234/v1' },
      'sys',
      [],
      [],
      100,
